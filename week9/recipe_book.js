...
function displayRecipes(RecipesToDisplay) {
  ...
  RecipesToDisplay.forEach(recipe => {
    ...
    const tagsHTML = recipe.tags.map(tag => `<button>${tag}</button>`).join('');
    ...
    RecipeCard.innerHTML = `
      ...
      <div class="recipe-content">
        ...
        <p><strong>Ingredients:</strong> ${recipe.recipeIngredient.join(', ')}</p>
        <p>${recipe.description}</p>
        <div class="rating">
          <span class="rating" role="img" aria-label="${recipe.rating} out of 5">${ratingStars}</span>
        </div>
      </div>
    `;
